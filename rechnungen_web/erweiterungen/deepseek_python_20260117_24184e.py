...
class GroqAnalyzer:
    ...
    def create_analysis_prompt(self, files: List[Dict]) -> str:
        ...
        # Granularität aus Config
        granularity = self.config.get('category_granularity', 'mittel')
        max_categories = {
            'wenig': 5,
            'mittel': 15,
            'viel': 30
    ...
    def validate_and_clean_result(self, result: Dict, files: List[Dict]) -> Dict:
        ...
        if "file_assignments" in result:
            assigned_files = {a["filename"] for a in result["file_assignments"]}
