...
def main():
        ...
        if uploaded_files:
            if st.button("🔄 Rechnungen verarbeiten", type="primary", use_container_width=True):
                with st.spinner("Verarbeite Rechnungen..."):
                    ...
                    st.session_state.summary_df = st.session_state.invoice_df.copy()
    ...
    # Tab 1: Übersicht
    with tab1:
        if st.session_state.invoice_df is not None and not st.session_state.invoice_df.empty:
    ...
    # Tab 4: Export
    with tab4:
        if (st.session_state.invoice_df is not None and 
            not st.session_state.invoice_df.empty and
            st.session_state.products_df is not None and 
            not st.session_state.products_df.empty):
